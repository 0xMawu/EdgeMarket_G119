...
// Minimal type shapes for the optional packages.
// Using explicit interfaces avoids a TS error when the packages aren't installed.
interface ExpoPermissionsResult { status: string }
interface ExpoPushTokenData { data: string }
interface NotificationsModule {
  getPermissionsAsync(): Promise<ExpoPermissionsResult>;
  requestPermissionsAsync(): Promise<ExpoPermissionsResult>;
...
export function usePushNotifications(): UsePushNotificationsResult {
  ...
  useEffect(() => {
    ...
    Notifications.getPermissionsAsync()
      .then(({ status }: ExpoPermissionsResult) => {
        setPermissionStatus(status as PermissionStatus);
        // If already granted, get the token immediately
        if (status === 'granted' && Device!.isDevice) {
          return Notifications!.getExpoPushTokenAsync().then((t: ExpoPushTokenData) => setPushToken(t.data));
        }
      })
