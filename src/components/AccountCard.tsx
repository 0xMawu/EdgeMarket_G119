// AccountCard - shows one leaderboard trader with their stats and open positions
...
export const AccountCard = memo(function AccountCard({
  account, isFollowing, onToggleFollow, expanded, onToggleExpand, rank,
}: Props) {
  ...
  // how many positions were hidden because user is on basic tier
  ...
  return (
    <View style={styles.card}>
        ...
        <View style={styles.headerLeft}>
          {rank !== undefined && (
            <View style={[
              ...
              : rank === 2 ? { backgroundColor: '#9ca3af' }
              : rank === 3 ? { backgroundColor: '#fb923c' }
          ...
          <View style={styles.flex}>
            ...
            <View style={styles.addressRow}>
              ...
              <Pressable onPress={() => Linking.openURL(`https://polygonscan.com/address/${account.address}`)} style={styles.iconBtn}>
      ...
      {/* Expand/collapse open positions */}
      ...
      {expanded && (
        <View style={styles.positionsList}>
          ...
          {/* show how many were hidden for basic users */}
          {truncatedCount > 0 && (
            <Text style={[styles.truncatedLabel, { color: colors.yellow }]}>
...
const makeStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  rankText: { color: '#fff', fontSize: 13, fontFamily: fonts.bold, fontWeight: '700' },
  flex: { flex: 1 },
  username: { color: colors.textPrimary, fontSize: 15, fontFamily: fonts.semiBold, fontWeight: '600', marginBottom: 4 },
  addressRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 },
  address: { color: colors.textFaint, fontSize: 11 },
  iconBtn: { padding: 4, backgroundColor: colors.cardBorder, borderRadius: 6 },
  ...
  statCell: { width: '47%', backgroundColor: colors.cardBorder, borderRadius: 10, padding: 10 },
  statLabel: { color: colors.textFaint, fontSize: 11, marginBottom: 4 },
  statValue: { color: colors.textPrimary, fontSize: 14, fontFamily: fonts.semiBold, fontWeight: '600' },
  expandBtn: {
  ...
  positionsBadge: {
    backgroundColor: 'rgba(168,85,247,0.2)', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 2,
  },
  positionsBadgeText: { color: '#d8b4fe', fontSize: 12 },
  positionsList: {
    marginTop: 12, borderTopWidth: 1, borderTopColor: colors.cardBorder, paddingTop: 8,
  },
  truncatedLabel: { fontSize: 12, fontFamily: fonts.medium, fontWeight: '500', textAlign: 'center', marginTop: 8, marginBottom: 4 },
});
